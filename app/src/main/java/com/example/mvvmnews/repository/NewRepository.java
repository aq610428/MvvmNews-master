...
public class NewRepository {
    ...
    public MutableLiveData<NewBean> getNews(String type, String appkey) {
        ...
        if (apiServices != null) {
            apiServices.getNews(type, appkey).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new BaseSubscriber<NewBean>() {
                        @Override
                        public void onNext(NewBean newBean) {
                            Log.d(TAG, "onNext: "+newBean.getResult().getData().get(0).getTitle());
